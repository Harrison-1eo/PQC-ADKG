...
/// `avss_deal` 用于测试 AVSS 协议的交易阶段的性能
/// `log_n` 为 `n` 的对数，`n` 为参与方的数量
/// `terminate_round` 为协议的终止轮数
fn avss_deal(log_n: usize, terminate_round: usize) {
    ...
    // interpolate_cosets 用于插值，`interpolate_cosets[i]` 为 `2^i` 次根的集合
    let mut interpolate_cosets = vec![Coset::new(
        1 << (log_t * 2 + CODE_RATE),
        Mersenne61Ext::random_element(),
    )];
    // 生成 `log_d` 个 `2^i` 次根的集合
    for i in 1..log_d {
        interpolate_cosets.push(interpolate_cosets[i - 1].pow(2));
    }

    // polynomial 为多项式 `f`，`f` 的次数为 `log_d`
...
/// `avss_verify` 用于测试 AVSS 协议的验证阶段的性能
/// `criterion` 为测试框架
/// `log_n` 为 `n` 的对数，`n` 为参与方的数量
/// `terminate_round` 为协议的终止轮数
